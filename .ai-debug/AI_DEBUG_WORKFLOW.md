...
11. Keep commands **reproducible** and easy to understand; add `#` comments where helpful. Prefer paths **relative to the root project directory** when you list commands (the scripts set the working directory to that folder).
...
- **Bake build failures** (`failed to execute bake: exit status 1`): `deploy/lib-nightmare-compose.sh` and `deploy/run-local.ps1` default **`COMPOSE_BAKE=false`**. For manual compose: `COMPOSE_BAKE=false docker compose -f deployment/docker-compose.yml build`.
