...
ensure_security_group() {
  local vpc_id="$1"
  local name="${ECS_SECURITY_GROUP_NAME:-argus-v2-ecs-workers}"
  local sg_id
  sg_id="$(
  ...
  if [[ -z "$sg_id" || "$sg_id" == "None" ]]; then
    sg_id="$(
      aws ec2 create-security-group \
        --region "$AWS_REGION" \
        --group-name "$name" \
        --description "argusV2 ECS worker tasks" \
        --vpc-id "$vpc_id" \
        --query 'GroupId' \
        --output text
    )"
...
ECS_TASK_EXECUTION_ROLE_ARN="${ECS_TASK_EXECUTION_ROLE_ARN:-$(ensure_role "${ECS_TASK_EXECUTION_ROLE_NAME:-argus-v2-ecs-task-execution}" true)}"
ECS_TASK_ROLE_ARN="${ECS_TASK_ROLE_ARN:-$(ensure_role "${ECS_TASK_ROLE_NAME:-argus-v2-ecs-task}" false)}"
...
ConnectionStrings__Postgres=Host=${private_ip};Port=5432;Database=argus_engine;Username=argus;Password=argus
ConnectionStrings__FileStore=Host=${private_ip};Port=5432;Database=argus_engine_files;Username=argus;Password=argus
argus__Postgres__MaxPoolSize=8
argus__FileStore__MaxPoolSize=4
ConnectionStrings__Redis=${private_ip}:6379
RabbitMq__Host=${private_ip}
RabbitMq__Username=argus
RabbitMq__Password=argus
...
Enumeration__SubdomainWordlistPath=/opt/argus/wordlists/subdomains.txt
EOF
...
write_env_assignment WORKER_SPIDER_SERVICE "${WORKER_SPIDER_SERVICE:-argus-worker-spider}"
write_env_assignment WORKER_ENUM_SERVICE "${WORKER_ENUM_SERVICE:-argus-worker-enum}"
write_env_assignment WORKER_PORTSCAN_SERVICE "${WORKER_PORTSCAN_SERVICE:-argus-worker-portscan}"
write_env_assignment WORKER_HIGHVALUE_SERVICE "${WORKER_HIGHVALUE_SERVICE:-argus-worker-highvalue}"
write_env_assignment WORKER_TECHID_SERVICE "${WORKER_TECHID_SERVICE:-argus-worker-techid}"
