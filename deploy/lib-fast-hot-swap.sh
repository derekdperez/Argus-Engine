...
# Publishes all changed .NET services in parallel on the host (using the SDK if available,
# or a shared SDK container), then simultaneously copies outputs into running containers
# and restarts them. Target wall-time for a source-only change: < 30s.
#
...
argus_host_dotnet() {
  if command -v dotnet >/dev/null 2>&1; then
    ...
    # We use a persistent named container so the SDK image is cached.
    local uid gid
    uid="$(id -u)"
    gid="$(id -g)"
    argus_docker run --rm \
...
argus_parallel_hot_copy_and_restart() {
  ...
  for service in "${services[@]}"; do
    ...
    (
      ...
      for p in "${copy_sub_pids[@]}"; do wait "$p"; done
