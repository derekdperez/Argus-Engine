...
nightmare_publish_service_for_hot_swap() {
  ...
  nightmare_docker run --rm \
    ...
    sh -lc "dotnet restore '$csproj' && dotnet publish '$csproj' -c Release -o '$out_rel' --no-restore /p:UseAppHost=false && mkdir -p '$out_rel/wwwroot/_framework' && blazor_web_js=\"\$(find /workspace/.nuget/packages /usr/share/dotnet -type f -name blazor.web.js 2>/dev/null | sort | tail -n 1 || true)\" && if [ -n \"\$blazor_web_js\" ]; then cp \"\$blazor_web_js\" '$out_rel/wwwroot/_framework/blazor.web.js'; fi"
}
