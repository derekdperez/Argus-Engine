...
nightmare_publish_service_for_hot_swap() {
  ...
  nightmare_docker run --rm \
    ...
    sh -lc "dotnet restore '$csproj' && dotnet publish '$csproj' -c Release -o '$out_rel' --no-restore /p:UseAppHost=false && if [ ! -f '$out_rel/wwwroot/_framework/blazor.web.js' ]; then echo 'FATAL: hot-swap publish did not produce blazor.web.js' >&2; exit 1; fi"
}
