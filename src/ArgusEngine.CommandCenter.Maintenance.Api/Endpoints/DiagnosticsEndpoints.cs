...
public static class DiagnosticsEndpoints
{
    ...
    private const string LegacyDiagnosticsKeyHeader = "X-Nightmare-Diagnostics-Key";
    ...
    private static bool TryAuthorizeDiagnostics(HttpContext http, IConfiguration config, out IResult? rejected)
    {
        ...
        {
            suppliedKey = http.Request.Headers[LegacyDiagnosticsKeyHeader].ToString();
        }
    ...
    private static DependencyCheck CheckStaticAssets(IConfiguration config)
    {
        ...
        return new DependencyCheck(
            ...
            new Dictionary<string, string>
            {
                ...
                ["verification"] = "Use deploy.py smoke to fetch these URLs from the running container.",
            });
    }
    ...
    private static (string Host, int Port) ResolveRabbitEndpoint(IConfiguration config)
    {
        ...
        if (lastColon > 0
            && lastColon < raw.Length - 1
            && int.TryParse(raw[(lastColon + 1)..], out var parsedPort))
        {
            host = raw[..lastColon];
            port = parsedPort;
        }
