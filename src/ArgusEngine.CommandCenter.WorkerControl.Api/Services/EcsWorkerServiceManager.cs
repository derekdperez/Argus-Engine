    ...
    EcsServiceNameResolver serviceNameResolver)
{
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(taskDefinition))
            throw new InvalidOperationException($"No active ECS task definition was found for family {family}. Run deploy.py ecs deploy or ./deploy deploy --ecs-workers once to register it.");
