...
{
    private static readonly Action<ILogger, int, string, Exception?> LogQueuedProbes =
        LoggerMessage.Define<int, string>(
            LogLevel.Information,
            new EventId(1, nameof(HighValuePathGuessConsumer)),
            "HighValuePaths: queued {Count} URL probes for host {Host}");
    ...
    public async Task Consume(ConsumeContext<AssetDiscovered> context)
    {
        ...
        if (published > 0)
            LogQueuedProbes(logger, published, host, null);
    }
