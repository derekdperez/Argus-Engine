...
public sealed class TechnologyCatalogLoader(ILogger<TechnologyCatalogLoader>? logger = null)
{
        ...
        foreach (var file in technologyDir.EnumerateFiles("*.json").OrderBy(f => f.Name, StringComparer.OrdinalIgnoreCase))
        {
            ...
            foreach (var property in doc.RootElement.EnumerateObject())
            {
                ...
                if (definition.Patterns.Count == 0
                    && definition.Implies.Count == 0
                    && definition.Requires.Count == 0
                    && definition.Excludes.Count == 0)
                {
                    // Keep metadata-only definitions because other technologies may imply them.
        ...
        ref int patternsSkipped)
    {
        ...
        AddPatternValues(name, element, "html", TechnologyConstants.HtmlSource, key: null, patterns, ref patternsCompiled, ref patternsSkipped);
        AddPatternValues(name, element, "url", TechnologyConstants.UrlSource, key: null, patterns, ref patternsCompiled, ref patternsSkipped);

        AddPatternValues(name, element, "scripts", TechnologyConstants.ScriptSource, key: null, patterns, ref patternsCompiled, ref patternsSkipped);
        AddPatternValues(name, element, "script", TechnologyConstants.ScriptSource, key: null, patterns, ref patternsCompiled, ref patternsSkipped);
        AddPatternValues(name, element, "scriptSrc", TechnologyConstants.ScriptSource, key: null, patterns, ref patternsCompiled, ref patternsSkipped);
