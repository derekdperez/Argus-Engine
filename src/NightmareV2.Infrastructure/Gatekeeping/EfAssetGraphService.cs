    ...
    ILogger<EfAssetGraphService> logger) : IAssetGraphService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (message.ParentAssetId is { } parentAssetId && parentAssetId != Guid.Empty)
        {
            var relationshipType = message.RelationshipType ?? AssetRelationshipType.Contains;
            var result = await UpsertRelationshipInsideTransactionAsync(
                    new AssetRelationshipDiscovered(
                        message.TargetId,
                        parentAssetId,
                        child.Id,
                        relationshipType,
                        message.IsPrimaryRelationship && relationshipType == AssetRelationshipType.Contains,
                        Confidence: 1.0m,
                        DiscoveredBy: message.DiscoveredBy,
                        DiscoveryContext: message.DiscoveryContext ?? "",
                        PropertiesJson: message.RelationshipPropertiesJson ?? "",
            ...
            relationshipUpdated = result.Updated;
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (result.RejectedReason is null)
            await tx.CommitAsync(cancellationToken).ConfigureAwait(false);
        else
