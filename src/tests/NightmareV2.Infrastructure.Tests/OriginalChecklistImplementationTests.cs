...
public sealed class OriginalChecklistImplementationTests
{
    ...
    public void DeploymentVersionIsIncrementedForThisPackage()
    {
        ...
        Assert.Equal("2.3.0", versionFile);
        Assert.Contains("<ArgusEngineDeploymentVersion>2.3.0</ArgusEngineDeploymentVersion>", targets);
        Assert.Contains("<AssemblyVersion>2.3.0.0</AssemblyVersion>", targets);
        Assert.Contains("<FileVersion>2.3.0.0</FileVersion>", targets);
    }
    ...
    {
        var root = FindRepositoryRoot();
        var page = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.CommandCenter", "Components", "Pages", "AssetAdmission.razor"));
        var endpoint = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.CommandCenter", "Endpoints", "AssetAdmissionDecisionEndpoints.cs"));
        var layout = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.CommandCenter", "Components", "Layout", "MainLayout.razor"));
        ...
        Assert.DoesNotContain("N2 Nightmare Command Center", layout);
    }
    ...
    {
        var root = FindRepositoryRoot();
        var orchestratorPath = Path.Combine(root, "src", "NightmareV2.Application", "Gatekeeping", "GatekeeperOrchestrator.cs");
    ...
    {
        var root = FindRepositoryRoot();
        var worker = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.Workers.Spider", "HttpRequestQueueWorker.cs"));
        var item = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.Domain", "Entities", "HttpRequestQueueItem.cs"));
        var store = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.Infrastructure", "FileStore", "EfHttpArtifactStore.cs"));
    ...
    {
        var root = FindRepositoryRoot();
        var retention = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.Infrastructure", "DataRetention", "DataRetentionWorker.cs"));
        var partition = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.Infrastructure", "DataRetention", "PostgresPartitionMaintenanceService.cs"));
        var outbox = File.ReadAllText(Path.Combine(root, "src", "NightmareV2.Infrastructure", "Messaging", "OutboxDispatcherWorker.cs"));
    ...
    public void ObservabilityIsWiredIntoExecutables()
    {
        ...
        {
            Path.Combine("src", "NightmareV2.CommandCenter", "Startup", "CommandCenterServiceRegistration.cs"),
            Path.Combine("src", "NightmareV2.Gatekeeper", "Program.cs"),
            Path.Combine("src", "NightmareV2.Workers.Spider", "Program.cs"),
            Path.Combine("src", "NightmareV2.Workers.Enum", "Program.cs"),
            Path.Combine("src", "NightmareV2.Workers.PortScan", "Program.cs"),
            Path.Combine("src", "NightmareV2.Workers.HighValue", "Program.cs"),
            Path.Combine("src", "NightmareV2.Workers.TechnologyIdentification", "Program.cs")
        })
        {
    ...
    public void RepoWideRenameMigrationScriptCoversRemainingDestructiveRenames()
    {
        ...
        foreach (var expected in new[]
        {
            "NightmareV2.Application",
            "ArgusEngine.Application",
            "NightmareDbContext",
            "ArgusDbContext",
            "NightmareRuntimeOptions",
            "ArgusRuntimeOptions",
            "NightmareV2.slnx",
            "ArgusEngine.slnx",
            "nightmare_v2"
